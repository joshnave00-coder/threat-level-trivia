...
- **Leaderboard Ranking & Per-Difficulty Storage** - The leaderboard now treats Hard and Medium as separate ladders that each hold up to 100 entries (instead of both fighting over a single shared top 100), so a strong Medium run is never bumped off by Hard runs. Within a ladder, entries rank by: score (highest first), then full-bank runs above ones that excluded Behind the Scenes, then most recent (a newer run outranks an older run on an otherwise-equal score, rewarding players who keep coming back). Hard always ranks above Medium - they're never blended by raw score. The combined "All" view stacks the full board Hard-first then Medium below (up to 200), while the Medium / Hard filter tabs each show up to 100.
- **Leaderboard Reserve (300-Score Retention)** - The public board still shows the top 100 per difficulty, but the server now retains up to **300 scores total** in storage instead of dropping everything past the displayed cut. Scores demoted below a tier's top 100 are kept "in reserve" (only the lowest-ranked are dropped once the 300 cap is exceeded), so they aren't lost and flow back onto the board if the display limit is raised later. The admin **Leaderboard** tab now lists *every* saved score and marks reserve ones with an amber **"In reserve · not shown"** pill plus a summary count (saved / on board / in reserve).
...
- Leaderboard submissions (`submitGlobalLeaderboardEntry`) now carry an `excludeBts` flag, persisted on each entry and surfaced in the render. `_save_leaderboard_entry` ranks entries by `(score desc, full-bank-before-excluded, most-recent)` - reusing each entry's existing millisecond `id` timestamp for recency, so no new field was needed. It retains every displayed entry (top 100 per tier) plus the next-best overflow up to a `STORAGE_CAP` of 300 total, dropping only the lowest-ranked beyond that. The client (`renderLeaderboardList`) applies its own view-specific sort via `_lbTopOfTier`, which caps each tier at 100 explicitly so reserve entries never leak onto the public board; the admin Leaderboard render flags any entry past its tier's top 100 as "in reserve."
