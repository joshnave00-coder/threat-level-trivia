...
- **Leaderboard** - The global Employee Records leaderboard tracks top qualifying scores. Hard and Medium are separate ladders that each hold up to 100 entries, so a strong Medium run is never bumped off by Hard runs. Within a ladder, entries rank by score, then full-bank runs above ones that excluded Behind the Scenes, then most recent (newer runs win an otherwise-equal tie, rewarding continued play). Hard always ranks above Medium: the All / Medium / Hard filter shows the "All" view stacked Hard-first then Medium below, or drills into either ladder. The public board shows the top 100 per difficulty, but the server retains up to 300 scores total in reserve, so scores demoted below the cut aren't lost and return if the display limit is raised later. Scores posted with "Behind the Scenes" excluded carry a small note under the player's name so they aren't assumed to be from the full bank.
...
- **Leaderboard Tab** - Lists every saved score (up to 300 total), with a saved / on-board / in-reserve summary. Scores past a difficulty's top 100 are marked "In reserve · not shown" (kept in storage, not on the public board). Remove entries with inappropriate usernames or fraudulent scores.
