...
The server handles both static file serving and a small JSON API that persists game data (disputes, ratings, leaderboard, feedback) to a local `data/` folder. That folder is created automatically on first run and is gitignored — it stays on your machine.
...
| Backend | Python 3 — `http.server` from the standard library |
...
Designed to deploy on any VPS (Hostinger, Akamai, etc.) behind a **Cloudflare Tunnel** for free HTTPS. Point a subdomain (e.g. `trivia.yourdomain.com`) at the tunnel — no reverse proxy config required.
